import { getClientError, getHttpStatus, refreshPlantFromPhoto } from '../../server/index';
import type { GenerateCarePlanInput } from '../../src/lib/aiSchema';

interface RefreshPlantFromPhotoInput extends GenerateCarePlanInput {
  image?: string;
  imageUrl?: string;
}

export default async function handler(request: any, response: any) {
  if (request.method !== 'POST') {
    response.setHeader('Allow', 'POST');
    return response.status(405).json({ error: 'Method not allowed' });
  }

  try {
    return response.status(200).json(await refreshPlantFromPhoto(request.body as RefreshPlantFromPhotoInput));
  } catch (error) {
    console.error('refresh-plant-from-photo failed:', error);
    return response.status(getHttpStatus(error)).json({ error: getClientError(error) });
  }
}
