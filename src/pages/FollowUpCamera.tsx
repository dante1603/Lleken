    ...
    <div className="bg-[#f8faf8] min-h-[100dvh] flex flex-col items-center justify-center p-6 relative font-sans">
      ...
      <div className="flex flex-col items-center text-center space-y-6 max-w-sm w-full bg-white p-8 rounded-[32px] shadow-sm border border-gray-100">
        <div className="w-24 h-24 bg-[#edf5f0] rounded-full flex items-center justify-center text-[#2e5c3a] mb-2">
          <span className="material-symbols-outlined text-[48px] fill">photo_camera</span>
        </div>
        ...
        <div className="w-full pt-4 flex flex-col gap-3">
          <button disabled={processing} onClick={() => cameraInputRef.current?.click()} className="w-full bg-[#2e5c3a] text-white py-4 flex items-center justify-center gap-2 shadow-sm font-medium rounded-2xl active:scale-95 transition-transform disabled:opacity-50">
            <span className="material-symbols-outlined fill">{processing ? 'hourglass_empty' : 'photo_camera'}</span>
            Tomar foto
          </button>

          <button disabled={processing} onClick={() => galleryInputRef.current?.click()} className="w-full bg-white text-gray-700 border border-gray-200 py-4 flex items-center justify-center gap-2 shadow-sm font-medium rounded-2xl active:scale-95 transition-transform active:bg-gray-50 disabled:opacity-50">
